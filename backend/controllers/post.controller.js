...
exports.getDetailPost = async (req, res) => {
    try {
        const detailPost = await Post.findById(req.params.id).select("title summary content_html slug status category tags image author publishdate").populate("category", "name").populate("author", "username");
        return res.status(200).json({
